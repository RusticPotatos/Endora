...
//! shown: Good morning. rustic isn't home, and E. Werner & M. Battaglia is at 6:30.
//! ```
//!
...
impl Pseudonyms {
    ...
    pub fn of(kinds: &BTreeMap<&str, Vec<String>>) -> Self {
        ...
        // Longest first: "E. Werner & M. Battaglia" must go before "Michael Battaglia", or
...
mod tests {
    ...
    fn table() -> Pseudonyms {
        ...
        kinds.insert(
            "person",
            vec!["rustic".to_owned(), "Michael Battaglia".to_owned()],
        );
        kinds.insert("place", vec!["Charlotte NC".to_owned()]);
        kinds.insert("event", vec!["E. Werner & M. Battaglia".to_owned()]);
        Pseudonyms::of(&kinds)
    }
    ...
    fn nothing_personal_is_in_what_leaves() {
        ...
        let brief = "rustic is not home; on the Family calendar: E. Werner & M. Battaglia \
                     at 2026-07-31 18:30:00; outside it is 69F";
        let sent = table().hide(brief);

        for real in ["rustic", "E. Werner", "Battaglia", "Charlotte"] {
    ...
    fn what_comes_back_is_the_persons_own_words_again() {
        let table = table();
        let sent = table.hide("rustic is not home; E. Werner & M. Battaglia at 18:30");
        let answered = format!("Good morning. {sent} — shall I set a reminder?");
        let shown = table.restore(&answered);
        assert!(shown.contains("rustic is not home"), "{shown}");
        assert!(shown.contains("E. Werner & M. Battaglia"), "{shown}");
        assert!(!shown.contains("<person"), "{shown}");
    }

    #[test]
    fn a_longer_name_is_replaced_before_the_shorter_one_inside_it() {
        // "Michael Battaglia" and "E. Werner & M. Battaglia" share a surname. Replacing the
        // shorter first would leave half a name in the outgoing text — the leak in disguise.
        let sent = table().hide("Michael Battaglia and E. Werner & M. Battaglia");
        assert!(!sent.contains("Battaglia"), "{sent}");
    }
